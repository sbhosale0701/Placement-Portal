...
// const sendJobNotification=require("../lib/sendEmail")
...
router.post("/jobs", jwtAuth, async (req, res) => {
  try {
    ...
    // ✅ Properly Saving Job with `await`
    ...
    const matchingApplicants = await User.find({domain:job.domain},"email");
    await sendJobNotification(matchingApplicants, job);
    res.status(201).json({ message: "Job posted & emails sent!" });
  ...
  if (user.type == "recruiter") {
    ...
    JobApplicant.findOne({ userId: user._id })
      .then((jobApplicant) => {
        ...
        if(data.domain){
          jobApplicant.domain=data.domain;
        }
        if (data.education) {
